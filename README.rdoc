...
For this workshop, we will be focusing on basic *Routes, Views and Controllers*

== Nitrous.IO Quick Setup

[1. Download Nitrous Pro for Chrome]
...
1. Let's get started by making a Rails app for a personal website. In the terminal, type
   <tt>$ rails new MySite</tt>
   ...
   The <tt>rails new <project_name></tt> is the starting point for every rails project</tt>

2. Run <tt>$ bundle install</tt>

   The <tt>bundle install</tt> command installed all the software packages needed by the new Rails    app. These software packages are called gems and they are listed in the file *Gemfile*.
   
3. Finally type <tt>rails s -b 0.0.0.0</tt>
    The <tt>rails server</tt> command started the Rails development server so that we could previe     w the app in the browser.
